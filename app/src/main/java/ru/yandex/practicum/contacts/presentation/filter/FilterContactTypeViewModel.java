...
public class FilterContactTypeViewModel extends BaseBottomSheetViewModel {
    ...
    private void updateFilterContactTypes() {
        String[] filtres = ContactType.getContactTypes();
        ArrayList<FilterContactTypeUi> filterContArr = new ArrayList<>();// создайте список типа FilterContactTypeUi и заполните его с помощью цикла forEach
        filterContArr.add(createAllSelectedItem(filtres));
        for (String filter : filtres){
           filterContArr.add(createFilterContactType(filter));
        }// forEach должен бежать по строковому массиву, который вы создали ранее

        filterContactTypesLiveDate.setValue(filterContArr);// вызовите меотод setValue() у переменной filterContactTypesLiveDate и передайте в качестве аргументы ваш список типа FilterContactTypeUi
    }
